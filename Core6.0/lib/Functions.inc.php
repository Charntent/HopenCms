...
function convert_catid_to_url_old($id,$start=null){
	global $is_html,$categorys,$sitepath,$list_html_rule,$setting,$web_local,$fengefu,$html_forder;
	if($categorys[$id]['cattype']=='diypage'){
		$phpscript = explode('|',$categorys[$id]['phpscript']);
		if($setting["rewrite_type"] == '程序伪静态' && $setting["UrlOptimization"] =='是'){
			$url = empty($phpscript[1])?$phpscript[0]:$phpscript[1];
		...
		if(empty($is_html)){
			if($categorys[$id]['cattype']=='article'){
				if($setting["rewrite_type"] == '程序伪静态' && $setting["UrlOptimization"] =='是' ){
					...
					return $web_local.DSS.$html_forder_this.$categorys[$id]['dir'].file_prefix();
					
				}else{
				    return $web_local.DSS."index.php?catid=".$id;
				}
			}elseif($categorys[$id]['cattype']=='page'){
				if($setting["rewrite_type"] == '程序伪静态' && $setting["UrlOptimization"] =='是'){
					 return $web_local.DSS.($categorys[$id]['dir']==''?"page".$fengefu.$id:$categorys[$id]['dir']).file_prefix();
				    ...
				    return $web_local.DSS."index.php?pageid=".$id;
				}
			}
		}else{
			$html_forder_this = $html_forder;
...
function a_c($url,$title,$class='',$target='',$onclick='',$stlye=''){
	if($onclick !='') {
	...
	echo '<a href="'.BASEURL."/".$url.'" class="'.$class.'" title="'.$title.'" '.$stlye.$onclick.'  target="'.$target.'">'.$title.'</a>';
}

function site_url($url =''){
	global $sitepath,$web_local,$rewrite,$setting,$fengefu,$defaultCtl;
	if($setting["rewrite_type"] == '程序伪静态' && $setting["UrlOptimization"] =='是'){
		if(empty($url)){
			return BASEURL;
		}else{
			$url = str_replace("/",$fengefu,$url);
			if($sitepath==""){
				return $web_local."/".$url.file_prefix();   
			}else
			   return $web_local.DSS.$sitepath."/".$url.file_prefix();
	    } 
	}else{
		$url = str_replace("/","=",$url);
		return "$defaultCtl?".$url;
	}
